#!/usr/bin/env python3
"""Append smooth Gaussian f/g auxiliary channels to an ABACUS .orb file."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Append one smooth Gaussian f channel and one smooth Gaussian g channel "
            "to an ABACUS .orb file by matching the peak position of the highest "
            "existing angular-momentum channel."
        )
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.25,
        help="Scale factor from f peak radius to g peak radius. Default: 1.25",
    )
    parser.add_argument(
        "--mode",
...
def main() -> int:
    ...
    if existing_lmax >= 4:
        raise SystemExit(
            "input already contains l >= 4 channels; use a physical base .orb without existing g "
            "or extend this script explicitly for that case"
        )
    ...
    add_f_channel = existing_lmax <= 2
    f_peak = reference_peak * args.f_scale if add_f_channel else None
    g_peak = (f_peak * args.g_scale) if add_f_channel else (reference_peak * args.g_scale)
    ...
    new_lmax = max(existing_lmax, 4)

    output_channels = list(orbital_file.channels)
    if add_f_channel and f_values is not None:
        output_channels.append(OrbitalChannel(l=3, n=counts[3] - 1, values=f_values))
    output_channels.append(OrbitalChannel(l=4, n=counts[4] - 1, values=g_values))
    ...
    report = {
        ...
        "reference_n": reference_channel.n,
        "reference_peak_r": round(reference_peak, 6),
        "added_channels": ["f", "g"] if add_f_channel else ["g"],
        "f_peak_r": round(f_peak, 6) if f_peak is not None else None,
        "g_peak_r": round(g_peak, 6),
        "f_primitives": f_primitives,
        "g_primitives": g_primitives,
        "window_start_fraction": args.window_start_fraction,
    }
